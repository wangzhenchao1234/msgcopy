...
- (void)jsq_configureFlowLayout
{
    ...
    _messageBubbleFont = MSGFont(15);
    _textLabel = [[TYAttributedLabel alloc] init];
    _textLabel.font = _messageBubbleFont;
    _textLabel.lineBreakMode = kCTLineBreakByCharWrapping;
    _textLabel.linesSpacing = 2;
    _textLabel.characterSpacing = 1.5;
...
- (CGSize)messageBubbleSizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([messageItem isMediaMessage]) {
        ...
        _textLabel.frame = CGRectMake(0, 0, maximumTextWidth, 40);
        _textLabel.text = [messageItem text];
        NSMutableArray *tmpArray = [NSMutableArray array];
        
        // 正则匹配图片信息
        __block BOOL hasImage = false;
        __block CGFloat maxWidth = 1;
        __block CGFloat maxHeight = 26;
        __block NSString *tmpStr = [messageItem text].mutableCopy;
        [CBRegularExpressionManager itemIndexesWithPattern:@"\\[[\\u4e00-\\u9fa5a-z]{1,3}\\]" inString:[messageItem text] usingBlock:^(NSString *name,NSRange range,  NSInteger idx, BOOL *const stop) {
            ...
            if (pic) {
                ...
                maxHeight = MAX(maxHeight,imageStorage.size.height);
                maxWidth += imageStorage.size.width+1;
                hasImage = true;
                tmpStr = [tmpStr stringByReplacingOccurrencesOfString:name withString:@""];
            }
        }];
        // 添加图片信息数组到label
        [_textLabel addTextStorageArray:tmpArray];
        [_textLabel sizeToFit];
        if (hasImage&&_textLabel.height<=maxHeight) {
            
            CGSize  stringSize = [tmpStr boundingRectWithSize:CGSizeMake(maximumTextWidth, CGFLOAT_MAX)
                                                                 options:(NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading)
                                                              attributes:@{ NSFontAttributeName : self.messageBubbleFont }
                                                                 context:nil].size;
            _textLabel.width = stringSize.width + maxWidth > maximumTextWidth?maximumTextWidth:(stringSize.width + maxWidth);
            [_textLabel sizeToFit];
            
        }else if (!hasImage){
            CGSize  stringSize = [[messageItem text] boundingRectWithSize:CGSizeMake(maximumTextWidth, CGFLOAT_MAX)
                                                      options:(NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading)
                                                   attributes:@{ NSFontAttributeName : self.messageBubbleFont }
                                                      context:nil].size;
            _textLabel.width = (stringSize.width + [[messageItem text] length]+1)> maximumTextWidth?maximumTextWidth:(stringSize.width + [[messageItem text] length]+1);
            [_textLabel sizeToFit];
        }
//        NSLog(@"%@",NSStringFromCGSize(_textLabel.size));
//        CGSize stringSize = CGRectIntegral(_textLabel.frame).size;
...
//        CGFloat finalWidth = MAX(stringSize.width + horizontalInsetsTotal, self.bubbleImageAssetWidth) + 2.0f;
        ...
        finalSize = CGSizeMake(_textLabel.width + 23, _textLabel.height + 14);
    }
...
- (UIAttachmentBehavior *)jsq_springBehaviorWithLayoutAttributesItem:(UICollectionViewLayoutAttributes *)item
{
    ...
    springBehavior.length = .0f;
    springBehavior.damping = .0f;
    springBehavior.frequency = .0f;

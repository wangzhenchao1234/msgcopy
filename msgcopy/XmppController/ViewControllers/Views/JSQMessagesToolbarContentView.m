...
@property (weak, nonatomic) IBOutlet UIView *emotionBarButtonContentView;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *emotionBarButtonContainerViewSpacingConstraint;

@property (weak, nonatomic) IBOutlet UIView *imageBarButtonContainerView;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *imageBarButtonContainerViewSpacingConstraint;
...
- (void)dealloc
{
    ...
    _emotionBarButtonItem = nil;
    _imageBarButtonItem = nil;
    _emotionBarButtonContentView = nil;
    _imageBarButtonContainerView = nil;
...
- (void)setEmotionBarButtonItem:(UIButton *)emotionBarButtonItem
{
    if (_emotionBarButtonItem) {
        [_emotionBarButtonItem removeFromSuperview];
    }
    
    if (!emotionBarButtonItem) {
        _emotionBarButtonItem = nil;
        self.emotionBarButtonContentView.hidden = YES;
        return;
    }
    
    if (CGRectEqualToRect(emotionBarButtonItem.frame, CGRectZero)) {
        emotionBarButtonItem.frame = self.emotionBarButtonContentView.bounds;
    }
    
    self.emotionBarButtonContentView.hidden = NO;
    
    [emotionBarButtonItem setTranslatesAutoresizingMaskIntoConstraints:NO];
    
    [self.emotionBarButtonContentView addSubview:emotionBarButtonItem];
    [self.emotionBarButtonContentView jsq_pinAllEdgesOfSubview:emotionBarButtonItem];
    [self setNeedsUpdateConstraints];
    _emotionBarButtonItem = emotionBarButtonItem;
}
- (void)setImageBarButtonItem:(UIButton *)imageBarButtonItem
{
    if (_imageBarButtonItem) {
        [_imageBarButtonItem removeFromSuperview];
    }
    
    if (!imageBarButtonItem) {
        _imageBarButtonItem = nil;
        self.emotionBarButtonContentView.hidden = YES;
        return;
    }
    
    if (CGRectEqualToRect(imageBarButtonItem.frame, CGRectZero)) {
        imageBarButtonItem.frame = self.imageBarButtonItem.bounds;
    }
    
    self.imageBarButtonContainerView.hidden = NO;
    
    [imageBarButtonItem setTranslatesAutoresizingMaskIntoConstraints:NO];
    
    [self.imageBarButtonContainerView addSubview:imageBarButtonItem];
    [self.imageBarButtonContainerView jsq_pinAllEdgesOfSubview:imageBarButtonItem];
    [self setNeedsUpdateConstraints];
    _imageBarButtonItem = imageBarButtonItem;
}

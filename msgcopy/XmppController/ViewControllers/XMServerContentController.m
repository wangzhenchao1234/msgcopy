...
@interface XMServerContentController ()
{
...
@implementation XMServerContentController

- (void)viewDidLoad {
    
    [super viewDidLoad];
    self.senderId = kCurUserName;
    self.usage_type = @"service";
    ...
    NSArray *messages = [[ContentManager sharedManager] getAllMessagesWithRoser:_roser type:@"chat" isServer:true];
    ...
    [[ContentManager sharedManager] updateUnreadMessagesWithRoser:_roser type:@"chat" isServer:true];
    /**
...
- (void)receiveMessage:(NSNotification *)notify
{
    ...
    if (![message.useagetype isEqualToString:@"service"]||![message.from isEqualToString:_roser]||![message.type isEqualToString:@"chat"]) {
        return;
    }
    [[ContentManager sharedManager] updateUnreadMessagesWithRoser:_roser type:@"chat" isServer:true];
                      ...
                      date:(NSDate *)date
{
    ...
    [bodyDict setObject:@"service" forKey:@"usage_type"];
...
{
    NSArray *messages = [[ContentManager sharedManager] getOlderMessagesWithRoser:_roser isServer:true type:@"chat" fromIndex:_messages.count];
    if ([messages count] == 0) {
        self.isAllLoad = true;

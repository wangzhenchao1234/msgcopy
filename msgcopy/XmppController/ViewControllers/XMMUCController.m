...
@interface XMMUCController ()<UIActionSheetDelegate>
{
...
@implementation XMMUCController
...
- (void)viewDidLoad {
    ...
    NSArray *meses = [[ContentManager sharedManager] getAlGrouplMessagesWithRoser:_roser];
    [meses enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if (message.souceType == SOMessageTypePhoto) {
            ...
            msg = [JSQMessage messageWithSenderId:message.master
          ...
          CRLog(@"视频 url = %@",message.sourceUrl);
              ...
              msg = [[JSQMessage alloc] initWithSenderId:message.master
    ...
    [[ContentManager sharedManager] updateUnreadMessagesWithRoser:_roser type:@"groupchat" isServer:false];
    /**
...
- (void)receiveMessage:(NSNotification *)notify
{
    ...
    if (![message.to isEqualToString:_roser]||![message.useagetype isEqualToString:@"normal"]||![message.type isEqualToString:@"groupchat"]) {
        return;
    }
    [[ContentManager sharedManager] updateUnreadMessagesWithRoser:_roser type:@"groupchat" isServer:false];
    /**
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (msg.isMediaMessage) {
            ...
            newMessage = [JSQMessage messageWithSenderId:message.master
                                             displayName:message.nickName
                                                  avatar:message.avatar
                                                   media:copyMediaData];
        }
                      ...
                      date:(NSDate *)date
{
    ...
    JSQMessage *message = [[JSQMessage alloc] initWithSenderId:kCurUser.userName
                                             senderDisplayName:kCurUser.firstName
    ...
    NSString *to = [NSString stringWithFormat:@"%@@conference.%@",toname,XMPP_SERVER_HOST_ME];
...
- (void)collectionView:(JSQMessagesCollectionView *)collectionView didTapAvatarImageView:(UIImageView *)avatarImageView atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (message.senderId) {
        ...
        content.userName = [NSString userName:message.senderId];
...
{
    NSArray *messages = [[ContentManager sharedManager] getOlderMessagesWithRoser:_roser isServer:false type:@"groupchat" fromIndex:_messages.count];
    if ([messages count] == 0) {
        self.isAllLoad = true;

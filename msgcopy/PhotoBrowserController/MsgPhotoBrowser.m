...
#import "MsgPhotoBrowserPrivate.h"
#import "PubRelView.h"
#import "ShareView.h"
...
@interface MsgPhotoBrowser ()

@end



@implementation MsgPhotoBrowser
...
- (id)initWithPub:(PubEntity *)pub {
    if ((self = [self init])) {
        _openType = DataOpenTypePub;
        NSArray *images = pub.article.imageSet.images;
        NSMutableArray *thums = [NSMutableArray new];
        for (KaokeImage *image in images) {
            MWPhoto *photo = [MWPhoto photoWithURL:CRURL(image.ourl)];
            photo.caption = image.descr;
            [thums addObject:photo];
        }
        _depreciatedPhotoData = thums;
        [self loadPubRel:pub];
    }
    return self;
}
- (id)initWithMsg:(ArticleEntity *)msg {
    if ((self = [self init])) {
        _openType = DataOpenTypeMsg;
        NSArray *images = msg.imageSet.images;
        NSMutableArray *thums = [NSMutableArray new];
        for (KaokeImage *image in images) {
            MWPhoto *photo = [MWPhoto photoWithURL:CRURL(image.ourl)];
            photo.caption = image.descr;
...
- (void)viewDidLoad {
    ...
    if (self.displayActionButton) {
        ...
        _shareButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"bt_share"] style:UIBarButtonItemStyleDone target:self action:@selector(doShare:)];
        _commentButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"bt_comment"] style:UIBarButtonItemStylePlain target:self action:@selector(doComment:)];
        UIButton *like = [UIButton buttonWithType:UIButtonTypeCustom];
        like.frame = CGRectMake(0, 0, 27, 27);
        [like addTarget:self action:@selector(doLike:) forControlEvents:UIControlEventTouchUpInside];
        [like setImage:[[UIImage imageNamed:@"bt_like"] imageWithTintColor:CRCOLOR_WHITE] forState:UIControlStateNormal];
        _likeButton = [[UIBarButtonItem alloc] initWithCustomView:like];
        _articleButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"bt_collece"] style:UIBarButtonItemStyleDone target:self action:@selector(doArticle:)];
...
//         We're first on stack so show done button
        ...
        _goBack.frame = CGRectMake(0, 0, 24, 24);
        [_goBack setImage:WhiteBackImage forState:UIControlStateNormal];
        [_goBack addTarget:self action:@selector(goBack:) forControlEvents:UIControlEventTouchUpInside];
        [_goBack setImageEdgeInsets:UIEdgeInsetsMake(0, -10, 0, 10)];
        _backButton = [[UIBarButtonItem alloc] initWithCustomView:_goBack];
        // Set appearance
    ...
    [items addObject:_articleButton];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    CRRootNavigation().navigationBar.titleTextAttributes = @{NSFontAttributeName:MSGFont(16),NSForegroundColorAttributeName:CRCOLOR_WHITE};
    // Status bar
...
#pragma clang diagnostic push
    ...
    if (_currentPageIndex < [self numberOfPhotos]) {
        ...
        [self setControlsHidden:false animated:false permanent:false];
        [self setToolsHidden:true animated:false];
    }
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    CRRootNavigation().navigationBar.titleTextAttributes = @{NSFontAttributeName:MSGFont(16),NSForegroundColorAttributeName:CRCOLOR_BLACK};
//    if ([self.navigationController.viewControllers objectAtIndex:0] != self &&
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_currentPageIndex != previousCurrentPage&&index < [self numberOfPhotos]) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (_currentPageIndex == [self numberOfPhotos]) {
        [self setNavsHidden:false animated:true];
        [self setToolsHidden:true animated:true];
    }else{
        if (![self areControlsHidden]) {
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            ...
            if (pubRels.count>0&&_currentPageIndex == _photoCount) {
                PubEntity *pub = pubRels[0];
                self.title = pub.pubrelTitle;
            }else{
...
- (void)showGrid:(BOOL)animated {
    ...
    _gridController.browser = (MWPhotoBrowser*)self;
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    // Status bar
    [self setNavsHidden:hidden animated:animated];
    
    // Toolbar, nav bar and captions
    // Pre-appear animation positions for iOS 7 sliding
    [self setToolsHidden:hidden animated:animated];
...
-(void)setNavsHidden:(BOOL)hidden animated:(BOOL)animated
{
...
-(void)setToolsHidden:(BOOL)hidden animated:(BOOL)animated
{
...
- (void)hideControlsAfterDelay {
    if (![self areControlsHidden]&&_currentPageIndex<[self numberOfPhotos]) {
        [self cancelControlHiding];
...
-(void)doLike:(id)sender
{
    ...
    CGRect frame = [_toolbar convertRect:_like.frame toView:self.view];
 ...
 *  @param pub
 */
-(void)loadPubRel:(PubEntity*)pub{
    
    CRWeekRef(self);
    [pub getPubRel:^(NSString *msg, NSInteger code, id data, NSString *requestURL) {
        pubRels = pub.pubrels;
        if (pubRels.count>0) {
            UIView *pubRelView = [__self buildPubRelView:pubRels];
            CGSize size = _pagingScrollView.contentSize;
            size.width += _pagingScrollView.width;
            _pagingScrollView.contentSize = size;
            [_pagingScrollView addSubview:pubRelView];
        }
...
-(UIView*)buildPubRelView:(NSArray*)rels
{
    PubRelView *view = [[PubRelView alloc] initWithFrame:CGRectMake(_pagingScrollView.contentSize.width + 10, 0, _pagingScrollView.width - 20, _pagingScrollView.height)];
    view.datas = rels;

...
#define TopInset 47.0f

#define ContentPan 10.0f
...
-(void)awakeFromNib
{
    ...
    _mediaView.y = _headView.y + 5;
    _mediaView.width = 3 * GridHeight + 2*GridPan;
    _mediaView.height = 0;
    _attrView.width = AppWindow.width - LeftInset - ContentPan;
    _nickView.x = _headView.width + _headView.x + 8;
    _nickView.y = _headView.y + 10;
    ...
    _separatorLine.height = 1/ScreenScale;
...
+(CGFloat)getHeightWithContent:(NSString *)content mediaCount:(NSInteger)count{
    ...
    VideoCommentCell *cell = [Utility nibWithName:@"VideoCommentCell" index:0];
    ...
    NSInteger rows = count%3==0?(count/3):(count/3+1);
    if (rows == 1) {
        mediaHeight += GridHeight + GridPan;
    }else if(rows == 2){
        mediaHeight += 2*GridHeight+ GridPan;
    }else if(rows >= 3){
        mediaHeight += 3*GridHeight + GridPan*2;

...
@implementation UIButton (MJWebButton)
- (void)setScaleImageWithURL:(NSString *)urlStr placeholder:(UIImage *)placeholder forState:(UIControlState)state{
    UIImage *plImage = [self image:placeholder scaleToSize:self.frame.size];
    [self setImage:plImage forState:state];
        ...
        } completed:^(UIImage *image, NSData *data, NSError *error, BOOL finished)
         {
             ...
             if (finished) {
                 if (image) {
                     ...
                     [self setImage:scaleImage forState:state];
                     image = nil;
                 }
             }
         }];
